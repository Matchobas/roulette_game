...
export function DeleteAllOptions({ wheelOptions, handleWheelOptions }: DeleteAllOptionsProps) {
  ...
  return (
    <>
      <button disabled={wheelOptions.length ? false : true} onClick={() => setIsDeleteConfirmationOpen(true)}>
      ...
      {isDeleteConfirmationOpen && (
        <div className='fixed w-[40%] h-1/4 flex flex-col items-center justify-center border-2 bg-slate-600 inset-0 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2'>
          <p className="text-white text-xl">Are you sure you want to delete all current options?</p>
